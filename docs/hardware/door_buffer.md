...
### The Problem
To give the [Modular Docks](modular_dock.md) the maximum amount of rigidity, they need to be supported above and below. Adding an extrusion [crossbar](#crossbar) solves that problem but also introduces another. With the crossbar in line with the frame the Voron idlers collide with the crossbar and moving the extrusion outside of the frame doesn't allow the door to be mounted anymore.
